...
#
# We build the application as Position Independent Executable are
# needed to take advantage of Address Space Layout Randomization,
# supported by some kernel versions. While ASLR can already be enforced
# for data areas in the stack and heap (brk and mmap), the code areas must
# be compiled as position-independent. Shared libraries already do this
# (-fPIC), so they gain ASLR automatically, but binary .text regions need to
# be build PIE to gain ASLR. When this happens, ROP attacks are much harder
# since there are no static locations to bounce off of during a memory
# corruption attack.
LDFLAGS += -Wl,-z,now,-z,relro,-z,defs -fPIE -pie
